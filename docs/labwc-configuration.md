...
> While you could instead explore simply running [py5.full_screen()](https://py5coding.org/reference/sketch_full_screen.html), this makes development on non-Pi devices difficult.
> Plus, you'll see entry-point scripts like [aquarium.py](../aquarium.py) are already configured to bound their scenes to the HyperPixel's display.
...
By default, `labwc` adds a title bar and thin border around every window (collectively called server-side decorations). 
For a py5 sketch running at 720×720, these decorations push the total window footprint beyond the physical display's dimensions, causing horizontal and vertical overflow.
...
This tells `labwc` not to draw any server-side decorations on any window, which is appropriate for a dedicated "kiosk" display. 

Reload the config one-time without rebooting using:
 
```bash
killall -s SIGHUP labwc
```
 
After making both changes, py5 sketches will render at the correct dimensions with no OS elements interfering.